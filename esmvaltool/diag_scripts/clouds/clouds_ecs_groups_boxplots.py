...
def get_provenance_record(ancestor_files):
    ...
    caption = (
        "Relative change per degree of warming averaged over thechosen region."
    )
...
def compute_diff_temp(input_data, group, var, dataset):
    ...
    tas_data_1 = select_metadata(
        input_data,
        short_name="tas",
        dataset=dataset_name,
        variable_group="tas_" + group[0],
    )
    tas_data_2 = select_metadata(
        input_data,
        short_name="tas",
        dataset=dataset_name,
    ...
    cube_diff = (
        100.0 * (cube_diff / iris.analysis.maths.abs(cube)) / cube_tas_diff
    )
