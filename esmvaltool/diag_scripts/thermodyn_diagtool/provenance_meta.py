...
def get_prov_map(attr, ancestor_files):
    ...
    caption = (
        f"Thermodynamic Diagnostic Tool - Monthly mean {attr[0]} (lat, lon) fields"
        f"for model {attr[1]}."
    )
...
def get_prov_transp(attr, ancestor_file):
    ...
    caption = (
        "Thermodynamic Diagnostic Tool - Annual mean zonally averaged"
        f" meridional {attr[0]} transports"
        f" for model {attr[1]}."
    )
