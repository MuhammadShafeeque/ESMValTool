...
def get_prof_and_plt_data(cfg, data, available_vars_min_tas):
    ...
    for attributes in data["ta"]:
        ...
        new_tacube = tacube.interpolate(sample_points, iris.analysis.Linear())
        ...
        for svar in available_vars_min_tas:
            ...
            plot_zonal_mean(
                cfg,
                svarcube.collapsed(["time"], iris.analysis.MEAN),
                dataset,
                "Zonal mean ",
                svarcube.long_name,
            )
...
def plot_zonal_mean(cfg, mean_cube, dataname, titlestr, variable):
    ...
    provenance_record = get_provenance_record(
        _get_sel_lvardata(cfg, dataname, [variable]),
        caption,
        ["mean"],
        ["global"],
    )
...
def plot_profiles(cfg, profiles, available_vars_min_tas, available_datasets):
    ...
    for svar in available_vars_min_tas:
        ...
        for iii, dataset in enumerate(available_datasets):
            plt.plot(
                (profiles[svar][dataset]).data,
                (profiles[svar][dataset]).coord("air_pressure").points / 100.0,
                label=dataset,
            )
            if iii == 0:
        ...
        axx.set_xlabel(onedat.long_name + " [" + unitstr + "]")
        axx.set_title("Average " + onedat.long_name + " profile")
        fig.tight_layout()
        figname = "fig_profile_" + onedat.long_name.replace(" ", "_")
        fig.savefig(get_plot_filename(figname, cfg), dpi=300)
        plt.close()

        provenance_record = get_provenance_record(
            _get_sel_files_var(cfg, svar),
            "Average " + onedat.long_name + " profile",
            ["mean"],
            ["global"],
        )
