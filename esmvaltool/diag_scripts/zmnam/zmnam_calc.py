...
def zmnam_calc(da_fname, outdir, src_props):
    ...
    # Lowpass filter
    zg_da_lp = butter_filter(zg_da, 1, lowcut=1.0 / 90, order=2)
    ...
    while idate < len(date):
        ...
        if idate == len(date) - 1 or (
            date[idate].month == mon and date[idate + 1].month != mon
        ):
    ...
    for i_lev in np.arange(len(lev)):
        ...
        if np.min(lat) > 0.0 and (lead_eof[max_lat] > lead_eof[min_lat]):
            lead_pc *= -1
            lead_eof *= -1
            index_name = "NAM"

        if np.min(lat) < 0.0 and (lead_eof[min_lat] > lead_eof[max_lat]):
            lead_pc *= -1
            lead_eof *= -1
            index_name = "SAM"
    ...
    with netCDF4.Dataset(fname, mode="w") as file_out:
        ...
        pcs_da_var = file_out.createVariable(
            "PC_da",
            "d",
            (
                "time",
    ...
    with netCDF4.Dataset(fname, mode="w") as file_out:
        ...
        pcs_mo_var = file_out.createVariable(
            "PC_mo",
            "d",
            (
                "time",
