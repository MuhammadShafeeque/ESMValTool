...
def zmnam_plot(
    ...
    for i_date in np.arange(len(time_mo)):
        yydate = netCDF4.num2date(time_mo, time_mo_uni, time_mo_cal)[
            i_date
        ].year
        mmdate = netCDF4.num2date(time_mo, time_mo_uni, time_mo_cal)[
            i_date
        ].month
    ...
    for i_lev in np.arange(len(lev)):
        ...
        plt.plot(
            bins,
            1.0 / (np.sqrt(2 * np.pi)) * np.exp(-(bins**2) / 2.0),
            linewidth=2,
            color="k",
            linestyle="--",
        )
        ...
        for j_lat in np.arange(len(lat)):
            for k_lon in np.arange(len(lon)):
                ...
                slope[j_lat, k_lon] = np.dot(
                    zg_mo[:, i_lev, j_lat, k_lon],
                    (pc_mo[:, i_lev])
    ...
    with netCDF4.Dataset(
        datafolder + "_".join(src_props) + "_regr_map_" + index_name + ".nc",
        mode="w",
    ) as file_out:
