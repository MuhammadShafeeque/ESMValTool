...
def ens_plots(
    ...
    namef = os.path.join(dir_output, f"ens_anomalies_{name_outputs}.nc")
    vartoplot, varunits, lat, lon = read_n_2d_fields(namef)
    print(f"vartoplot dim: (numens x lat x lon)={vartoplot.shape}")
    ...
    namef = os.path.join(dir_output, f"legend_{name_outputs}.txt")
    ...
    fsize = int(min(max(4.0 / ypos * 15, 10), 32))
    ...
    for ifig in range(nfigs):
        ...
        print(
            "A ",
            plot_type,
            f" figure for the selected fields saved in {dir_plot}",
        )
...
def round_up(x, sig=2):
    ...
    dig = pow(10.0, sig - int(math.floor(math.log10(abs(x)))) - 1)
