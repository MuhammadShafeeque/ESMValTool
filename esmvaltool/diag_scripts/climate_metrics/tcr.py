...
def _get_anomaly_cube(onepct_cube, pi_cube):
    ...
    onepct_cube.data -= (
        reg.slope * pi_cube.coord("year").points + reg.intercept
    )
    ...
    onepct_cube.attributes["anomaly"] = (
        "relative to linear fit of piControl run"
    )
...
def calculate_tcr(cfg):
    ...
    for dataset_name, anomaly_cube in anomaly_cubes.items():
        tas_2x = (
            anomaly_cube[START_YEAR_IDX:END_YEAR_IDX]
