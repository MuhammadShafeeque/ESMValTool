    ...
    caption = (
        ...
        "experiment.".format(
            ...
            (
                "global mean"
                if NDIMS.get("tas") == 1
                else "{}-dependent".format(" and ".join(COORDS["tas"]))
            ),
...
def _create_regression_plot(
    ...
    # Plot data
    title = (
        f"{FEEDBACK_PARAMETERS.get(var, var)} TOA radiance for {dataset_name}"
    )
    filename = f"{var}_regression_{dataset_name}"
    if description is not None:
        title += f" ({description})"
        filename += f"_{description.replace(' ', '_')}"
    plot_path = get_plot_filename(filename, cfg)
    text = rf"r = {reg.rvalue:.2f}, {NICE_SYMBOLS.get(var, var)} = {reg.slope:.2f}"
    if var in ("rtmt", "rtnt"):
        text += f", F = {reg.intercept:.2f}, ECS = {-reg.intercept / (2.0 * reg.slope):.2f}"
    plot.scatterplot(
        [tas_cube.data, x_reg],
        [cube.data, y_reg],
        plot_path,
        ...
        axes_functions={
            ...
            "set_title": title,
            "set_xlabel": f"tas / {NICE_UNITS.get(tas_cube.units.origin, 'K')}",
            "set_ylabel": f"{var} / {NICE_UNITS.get(cube.units.origin, 'unknown')}",
            "set_xlim": [0.0, 8.0],
            "text": {
                "args": [0.05, 0.9, text],
                "kwargs": {
...
def _get_anomaly_data(input_data, year_idx=None):
    ...
    logger.info(
        "Found %iD 'tas' data and %iD radiation data%s",
        NDIMS.get("tas"),
        NDIMS.get("rad"),
        msg,
    )
...
def _write_scalar_data(data, ancestor_files, cfg, description=None):
    ...
    for idx, var_attr in enumerate(var_attrs):
        ...
        if NDIMS["rad"] == 1:
            io.save_scalar_data(
                {d: data[idx][d].data for d in data[idx]},
                path,
                var_attr,
...
def calculate_ecs(input_data, cfg, description=None):
    ...
    for dataset in select_metadata(input_data, short_name="tas"):
        ...
        coords = [
            (coord, idx - 1)
            for (idx, coord) in enumerate(rtnt_cube.coords(dim_coords=True))
            if coord.name() != "time"
        ]
...
def plot_feedback_parameters(input_data, cfg, description=None):
    ...
    for var, datasets in group_metadata(input_data, "short_name").items():
        ...
        for dataset in datasets:
            dataset_name = dataset["dataset"]
            tas_data = select_metadata(
                input_data,
                short_name=_get_tas_var(dataset_name, var),
                dataset=dataset_name,
            )
            if not tas_data:
                raise ValueError(
...
def plot_regressions(input_data, cfg, description=None):
    ...
    for var, datasets in group_metadata(input_data, "short_name").items():
        ...
        for dataset in datasets:
            ...
            (plot_path, reg) = _create_regression_plot(
                tas_cube,
                dataset["cube"],
                dataset_name,
                cfg,
            ...
            netcdf_path = _create_regression_file(
                tas_cube,
                dataset["cube"],
                dataset_name,
                cfg,
...
def preprocess_data(cfg, year_idx=None):
    ...
    if cfg["calculate_mmm"]:
        ...
        if cfg["only_consider_mmm"]:
            ...
            input_data = [
                d for d in input_data if d["dataset"] == "MultiModelMean"
            ]
