...
def compute_overall_mean(
    ...
    overall_mean = (
        normalized.to_array(dim="variable_group")
        .weighted(weights_selected)
        .mean("variable_group")
    )
...
def combine_ensemble_members(
    ...
    assert len(dimensions) <= 2, (
        "dimensions can contain a maximum of two strings"
    )
    ...
    for dimension in dimensions:
        if dimension in dataset.dims:
            ...
            dataset = (
                dataset.groupby(model)
                .mean(keep_attrs=True)
                .rename({"group": dimension})
            )
...
def calculate_weights(
    ...
    independence_core_dims = (
        ...
        else ["model_ensemble", "model_ensemble_reference"]
    )
