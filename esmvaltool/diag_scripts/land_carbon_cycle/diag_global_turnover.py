...
def _get_diagonal_colorbar_info():
    ...
    cb_info_diagonal["tickBounds"] = np.concatenate(
        (
            [1],
            np.linspace(8, 16, num=10)[:-1],
            np.linspace(16, 32, num=10)[:-1],
            np.linspace(32, 64, num=10)[:-1],
...
def _get_fig_config(diag_config):
    ...
    """
    nmodels = (
        len(group_metadata(diag_config["input_data"].values(), "dataset")) + 1
    )
    w_pl = 1.0 / nmodels
    h_pl = w_pl
    aspect_map = 0.5
...
def _get_ratio_colorbar_info():
    ...
    cb_info_ratio["tickBounds"] = np.concatenate(
        (
            np.geomspace(0.2, 0.25, num=num_col),
            np.geomspace(0.25, 0.33, num=num_col),
            np.geomspace(0.33, 0.5, num=num_col),
            np.geomspace(0.5, border, num=num_col),
            np.linspace(border, 1 / border, num=num_gr),
            np.geomspace(1 / border, 2, num=num_col),
...
def _plot_multimodel_agreement(
    ...
    nmodels = len(models)
    dat_tau_full = (
        np.ones((nmodels, np.shape(tau_obs)[0], np.shape(tau_obs)[1]))
...
def main(diag_config):
    ...
    base_name = "{title}_{source_label}_{grid_label}".format(
        title=global_tau_obs["grid"]["tau_ctotal"].long_name,
    ...
    for model_name, model_dataset in model_data_dict.items():
        ...
        base_name_mod = "global_{title}_{source_label}_{grid_label}".format(
            title=global_tau_obs["grid"]["tau_ctotal"].long_name,
        ...
        model_cubes = [
            c
            for c in global_tau_mod["grid"].values()
            if isinstance(c, iris.cube.Cube)
        ]
        obs_cubes = [
            c
            for c in global_tau_obs["grid"].values()
            if isinstance(c, iris.cube.Cube)
        ]
