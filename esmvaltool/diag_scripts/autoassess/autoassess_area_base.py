...
def _import_package(area):
    ...
    available_areas = [
        ...
        "conservation",
        "globaltrop",
        "land_surface_surfrad",
        "land_surface_snow",
        "land_surface_soilmoisture",
        "land_surface_permafrost",
    ]
...
def _get_filelists(cfg):
    ...
    for filename in cfg["input_data"]:
        ...
        if (
            additional_metrics
            and base_file.split("_")[1] in cfg["additional_metrics"]
        ):
...
def _setup_input(cfg):
    ...
    logger.info(
        "Files for control model for metrics: %s",
        metrics_dict["control_model"],
    )
    logger.info(
...
def run_area(cfg):
    ...
    area_out_dir = create_output_tree(
        run_obj["out_dir"],
        run_obj["suite_id1"],
        run_obj["suite_id2"],
        run_obj["_area"],
