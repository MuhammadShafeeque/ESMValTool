...
def plot_timehgt(cube, levels, title, log=False, ax1=None):
    ...
    ax1.xaxis.set_label_text(new_unit)
...
def plot_uwind(cube, month, filename):
    ...
    title = f"Zonal mean zonal wind ({month})"
    fig = plt.figure()
...
def qbo_metrics(run, ucube, metrics):
    ...
    p30 = iris.Constraint(air_pressure=3000.0)
...
def tpole_metrics(run, tcube, metrics):
    ...
    metrics["50 hPa temperature: 60N-90N (DJF)"] = djf_polave.data - 180.0
    metrics["50 hPa temperature: 60N-90N (MAM)"] = mam_polave.data - 180.0
    metrics["50 hPa temperature: 90S-60S (JJA)"] = jja_polave.data - 180.0
    metrics["50 hPa temperature: 90S-60S (SON)"] = son_polave.data - 180.0
...
def q_mean(cube):
    ...
    # TODO magic numbers
    return (1000000.0 * 29.0 / 18.0) * qmean.data  # ppmv
...
def teq_metrics(run, tcube, metrics):
    ...
    p100 = iris.Constraint(air_pressure=10000.0)
...
def q_metrics(run, qcube, metrics):
    ...
    p70 = iris.Constraint(air_pressure=7000.0)
...
def calc_merra(run):
    ...
    tmerra = t.data  # K
    # TODO magic numbers
    qmerra = (1000000.0 * 29.0 / 18.0) * q.data  # ppmv
...
def calc_erai(run):
    ...
    # Create return values
    terai = t.data  # K
    qerai = (1000000.0 * 29.0 / 18.0) * q.data  # ppmv
    return terai, qerai
