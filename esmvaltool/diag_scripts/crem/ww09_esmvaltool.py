...
def main(cfg):
    ...
    for dataset in grouped_input_data:
        ...
        for var in ww_vars_plus:
            ...
            if not selection:
                logger.info(
                    "%s: no data for variable snc found, trying "
                    "variable snw instead",
                    dataset,
                )
                pointers[key_nc] = ""
                pointers[key_var] = ""
        ...
        if missing_vars:
            ...
            logger.error(
                "error: the following variables are not available: %s",
                printlist,
            )
...
def read_and_check(srcfilename, varname, lons2, lats2, time2):
    ...
    try:
        data = np.ma.masked_equal(src_data, src_data._FillValue)
...
def crem_calc(pointers):
    ...
    crem_pd = (
        (
            np.sum(r_crem_pd[0, :] ** 2)
            + np.sum(r_crem_pd[1, :] ** 2)
            + np.sum(r_crem_pd[2, 0:5] ** 2)
        )
        / 20.0
    ) ** 0.5
