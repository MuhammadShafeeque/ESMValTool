...
def plot_rain_and_wind(cfg, dataname, data, future_exp):
    ...
    if data["ar_diff_rain"].ndim == 3:
        plt.contour(
            data["lons"],
            data["lats"],
            np.std(data["ar_diff_rain"], axis=2),
            [0.2, 0.4, 0.6],
            transform=cart.PlateCarree(),
            colors="w",
        )
    ...
    fig.tight_layout()
    fig.savefig(get_plot_filename(dataname + "_li17natcc_fig1a", cfg), dpi=300)
    plt.close()

    if data["ar_diff_rain"].ndim == 2:
        caption = (
            dataname
            + ": Changes in precipitation (colour shade, "
            + "mm d-1) "
    ...
    with ProvenanceLogger(cfg) as provenance_logger:
        provenance_logger.log(
            get_plot_filename(dataname + "_li17natcc_fig1a", cfg),
            provenance_record,
        )
...
def plot_rain(cfg, titlestr, data, lats, lons):
    ...
    titlestr = (
        titlestr
        + " Box displays the area used to define the "
...
def plot_2dcorrelation_li(cfg, reg2d, lats, lons):
    ...
    iris.save(
        cube_to_save_ploted(
            ...
            lats,
            lons,
            {"var_name": "corr", "long_name": "Correlation", "units": "1"},
        ),
        target=diagnostic_file,
    )
...
def plot_reg_li(cfg, data_ar, future_exp):
    ...
    axx.text(8.1, 0.01, f"r = {reg.rvalue:.2f}")
...
def plot_reg_li2(cfg, datasets, mdiff_ism, mdiff_ism_cor, hist_ism):
    ...
    iris.save(
        cube_to_save_scatter(
            ...
            {
                "var_name1": "rd_pr",
                "long_name1": "Relative Precipitation " + "Change",
                "units1": "percent K-1",
                "var_name2": "corr_pr",
...
def correct_li(data, lats, lons, reg):
    ...
    for iii in range(len(data["datasets"])):
        ...
        ar_diff_cor[:, :, iii] = (
            data["ar_diff_rain"][:, :, iii] - proj_err[:, :, iii]
        )
...
def main(cfg):
    ...
    for dataset_path in data:
        ...
        cube = cube[
            np.where(np.absolute(cube.coord("month_number").points - 7) <= 2)
        ]
    ...
    plot_reg_li2(
        ...
        data_ar["datasets"],
        data_ar["mism_diff_rain"],
        data_ar2["mism_diff_cor"],
        data_ar2["mism_hist_rain"],
    )

    plot_rain(
        cfg,
        "Multi-model mean rainfall change due to model error",
        np.mean(data_ar2["proj_err"], axis=2),
        lats,
        lons,
    )
    plot_rain(
        cfg,
        "Corrected multi-model mean rainfall change",
        np.mean(data_ar2["ar_diff_cor"], axis=2),
        lats,
        lons,
    )
