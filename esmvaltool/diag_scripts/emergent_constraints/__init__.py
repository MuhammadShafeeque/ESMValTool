...
def _get_external_cube_list(external_file):
    ...
    if external_file is None:
        return iris.cube.CubeList()
    with open(external_file) as infile:
...
def plot_individual_scatterplots(
    ...
    for feature in training_data.x.columns:
        ...
        for idx, group in enumerate(groups):
            ...
            filename = (
                f"scatterplot_{basename}_{feature}_{group.replace(', ', '-')}"
            )
            plot_path = get_plot_filename(filename, cfg)
            plt.savefig(plot_path, **cfg.get("savefig_kwargs", {}))
    ...
    for feature in training_data.x.columns:
        ...
        if len(groups) > 1 and cfg.get("combine_groups", False):
            axes = _create_regplot(
                ...
                mean_kwargs=(
                    None
                    if not cfg.get("plot_regression_line_mean")
                    else {"color": COLOR_COMBINED_GROUPS, "marker": "o"}
                ),
            )
            axes = _create_pred_output_plot(
                x_data,
    ...
    for feature in training_data.x.columns:
        ...
        for idx, group in enumerate(groups):
            ...
            logger.info(
                ...
                "(%i%% confidence level), R2 = %f, p = %f",
                label,
                feature,
                group,
                y_mean,
                y_error,
                int(100.0 * cfg["confidence_level"]),
                reg.rvalue**2,
                reg.pvalue,
            )
        ...
        provenance_record = get_provenance_record(
            ...
            [feature, label],
            caption=(
                f"{attributes[feature]['plot_title']}: Probability "
                f"densitiy of {label}."
            ),
...
def constraint_info_array(
    ...
    """
    (y_lin, y_pdf, reg) = _get_target_pdf(
        x_data,
        y_data,
        obs_mean,
        obs_std,
