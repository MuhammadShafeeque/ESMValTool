...
def plot_temperature_anomaly(cfg, tas_cubes, lambda_cube, obs_name):
    ...
    filename = f"temperature_anomaly_{obs_name}"
    ...
    provenance_record = get_provenance_record(
        f"Simulated change in global temperature from {project} models (coloured "
        f"lines), compared to the global temperature anomaly from the {obs_name} "
...
def plot_psi(cfg, psi_cubes, lambda_cube, obs_name):
    ...
    filename = f"temperature_variability_metric_{obs_name}"
    netcdf_path = get_diagnostic_filename(filename, cfg)
...
def plot_emergent_relationship(cfg, psi_cube, ecs_cube, lambda_cube, obs_cube):
    ...
    logger.info(
        "Found emergent relationship with slope %.2f (R2 = %.2f)",
        lines["slope"],
        lines["rvalue"] ** 2,
    )
...
def plot_pdf(cfg, ecs_lin, ecs_pdf, ecs_cube, obs_name):
    ...
    provenance_record = get_provenance_record(
        ...
        f"that arise from equal weighting of the {project} models in 0.5 K bins.",
        ["mean"],
        ["other"],
        _get_ancestor_files(cfg, obs_name),
    )
...
def plot_cdf(cfg, ecs_lin, ecs_pdf, ecs_cube, obs_name):
    ...
    project = _get_project(cfg)
    provenance_record = get_provenance_record(
        f"The CDF for ECS. The horizontal dot-dashed lines show the {int(confidence_level * 100)}% "
        "confidence limits. The orange histograms show the prior "
        f"distributions that arise from equal weighting of the {project} models in "
        "0.5 K bins.",
        ["mean"],
        ["other"],
        _get_ancestor_files(cfg, obs_name),
...
def get_ecs_range(cfg, ecs_lin, ecs_pdf):
    ...
    ecs_cdf = ec.cdf(ecs_lin, ecs_pdf)
    ecs_index_range = np.where((ecs_cdf >= conf_low) & (ecs_cdf <= conf_high))[
        0
    ]
...
def main(cfg):
    ...
    for obs_name in psi_obs:
        ...
        logger.info(
            "Observational constraint: Ψ = (%.2f ± %.2f) K",
            np.mean(obs_cube.data),
            np.std(obs_cube.data),
        )
        logger.info(
            "Constrained ECS range: (%.2f - %.2f) K with best estimate %.2f K",
            ecs_range[1],
            ecs_range[2],
            ecs_range[0],
        )
