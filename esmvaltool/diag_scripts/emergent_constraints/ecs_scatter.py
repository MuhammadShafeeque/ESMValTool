...
def _similarity_metric(cube, ref_cube, metric):
    ...
    return metric_func(
        np.ma.array(new_data, mask=mask).compressed(),
...
def brient_shal(grouped_data, cfg):
    ...
    for dataset_name, datasets in grouped_data.items():
        ...
        [cf_950, cf_850] = _get_weighted_cloud_fractions(
            cl_cube,
            zg_cube,
            [(100000, 90000), (90000, 80000)],
            n_jobs=cfg["n_jobs"],
        )
...
def volodin(grouped_data, _):
    ...
    for dataset_name, datasets in grouped_data.items():
        ...
        for dataset in datasets:
            if (
                "trop" in dataset["variable_group"]
        ...
        for dataset in datasets:
            if (
                "midlat" in dataset["variable_group"]
