...
def set_grid_pw_reg_obs(cubes, meas_tub_rsnstcsnorm, meas_tub_prw):
    ...
    if np.min(np.array([len(meas_tub_rsnstcsnorm), len(meas_tub_prw)])) > 0:
        ...
        for kmeas_tub_prw in meas_tub_prw:
            data_prw_obs[kmeas_tub_prw[0]] = cubes[kmeas_tub_prw]
            (grid_pw["yobs"])[kmeas_tub_prw[0]] = make_grid_prw(
                grid_pw["x"],
...
def plot_deangelis_fig3a(cfg, dataset_name, data, reg_prw, reg_obs):
    ...
    reg_prw_dict["text"] = (
        dataset_name + rf", $\delta$rsnst / $\delta$prw ={reg_prw.slope:.2f}"
    )
    ...
    axx.plot(data["x"], data["ypic"], linestyle="none", color="k", marker="d")

    ccc = 0.0
    for kobs in reg_obs.keys():
        reg_prw_dict["y_obs"] = (
            reg_obs[kobs].slope * reg_prw_dict["x"] + reg_obs[kobs].intercept
        )
        reg_prw_dict["text_obs"] = (
            "CERES-EBAF/"
            + kobs
            + r", $\delta$rsnst / $\delta$"
            + f"prw ={reg_obs[kobs].slope:.2f}"
        )
        axx.plot(
            reg_prw_dict["x"],
            reg_prw_dict["y_obs"],
...
def plot_deangelis_fig4(cfg, data_model, mdrsnstdts, prw):
    ...
    provenance_record = get_provenance_record(
        _get_sel_files_var(cfg, ["prw", "rsnst"]),
        caption,
        ["other"],
        ["global"],
    )
...
def plot_deangelis_fig3b4(cfg, data_model, reg_prw_obs):
    ...
    for iii, modelkey in enumerate(data_model.keys()):
        ...
        style = (
            select_metadata(cfg["input_data"].values(), dataset=modelkey)
        )[0]["project"]
    ...
    if reg_prw_obs:
        ...
        for iii, modelkey in enumerate(reg_prw_obs.keys()):
            (prw["min"])[iii] = (
                reg_prw_obs[modelkey].slope
                - 2.0 * reg_prw_obs[modelkey].stderr
            )
            (prw["max"])[iii] = (
                reg_prw_obs[modelkey].slope
                + 2.0 * reg_prw_obs[modelkey].stderr
    ...
    axx.plot(
        np.linspace(0.0, 0.15, 2),
        reg.slope * np.linspace(0.0, 0.15, 2) + reg.intercept,
        linestyle="solid",
        color="r",
        label=f"Fit (r ={reg.rvalue:.2f})",
    )
    ...
    provenance_record = get_provenance_record(
        _get_sel_files_var(cfg, ["prw", "tas", "rsnst", "rsnstcs"]),
        caption,
        ["other"],
        ["global"],
    )
...
def substract_and_reg_deangelis(cfg, cubes, grid_pw, reg_prw_obs):
    ...
    for model_run in model_tub_tas_pi:
        ...
        data_rsnstcsnorm_pic = cubes[
            (model_run[0], "rsnstcsnorm", model_run[2])
        ]
        data_tas = (
            (cubes[(model_run[0], model_run[1], "abrupt4xCO2")]).data
            - (cubes[model_run])
        )
        data_rsnstcs = (
            (cubes[(model_run[0], "rsnstcs", "abrupt4xCO2")])
        ...
        plot_deangelis_fig3a(cfg, model_run[0], grid_pw, reg_prw, reg_prw_obs)
