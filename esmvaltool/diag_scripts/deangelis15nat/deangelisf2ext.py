...
def _calculate_regression_sa(sa_dict):
    ...
    reg_dict["y_sa"] = (
        reg_dict["sa"].slope * np.linspace(0.2, 1.4, 2)
...
def _set_axx_fig2a(cfg, axx, m_all, reg_dict, sa_dict):
    ...
    axx.set_xticklabels(
        ("dlvp/dtas", "drsnst/dtas", "rsnstcs/dtas"),
        rotation=45,
        ha="right",
...
def _set_axx_fig2b(axx, cfg, reg_dict, datasets, sa_dict):
    ...
    for iii, model in enumerate(datasets):
        proj = (select_metadata(cfg["input_data"].values(), dataset=model))[0][
            "project"
        ]
    ...
    axx.text(
        0.9,
        2.75,
        "Fit (r={:.2f}, ".format(reg_dict["sa"].rvalue)
...
def _set_axx_exfig2b(axx, cfg, datasets, reg_dict, sa_dict):
    ...
    axx.text(
        0.85,
        1.1,
        "Fit (r={:.2f}, ".format(reg_dict["rsnst"].rvalue)
...
def plot_slope_regression(cfg, data_dict):
    ...
    list_dict["name"] = [
        ...
        {
            "var_name": "drsnst_divby_dtas",
            "long_name": "Temperature mediated " + "shortwave absorption",
            "units": "W m-2 K-1",
        },
        {
            "var_name": "drsnstcs_divby_dtas",
...
def plot_slope_regression_all(cfg, data_dict, available_vars):
    ...
    m_all = np.array(
        [
            np.mean(data_model[:, 3]),
            np.mean(data_model[:, 0]),
            np.mean(data_model[:, 1]),
...
def plot_rlnst_regression(cfg, dataset_name, data, variables, regs):
    ...
    yreg_dict = {}
    yreg_dict["rlnst"] = (
        regs["rlnst"].slope * np.linspace(0.0, 7.0, 2)
        + regs["rlnst"].intercept
    )
    yreg_dict["rsnst"] = (
        regs["rsnst"].slope * np.linspace(0.0, 7.0, 2)
        + regs["rsnst"].intercept
    )
    yreg_dict["hfss"] = (
        regs["hfss"].slope * np.linspace(0.0, 7.0, 2) + regs["hfss"].intercept
    )
    yreg_dict["lvp"] = (
        regs["lvp"].slope * np.linspace(0.0, 7.0, 2) + regs["lvp"].intercept
    )

    yreg_dict["lab_lvp"] = (
        "dlvp/dtas = {:.2f}, ".format(regs["lvp"].slope)
    ...
    caption = (
        " Demonstration of the Gregory method for "
        + dataset_name
        + ". Global-mean annual anomalies ("
        + ABRUPT4XCO2
        + "—"
        + PICONTROL
        + " in atmospheric energy budget terms "
    ...
    list_dict["name"] = [
        {"var_name": "tas", "long_name": "2-m air temperature", "units": "K"},
        {
            "var_name": "lvp",
            "long_name": "Latent heat release " + "from precipitation",
            "units": "W m-2",
        },
        {
            "var_name": "rlnst",
