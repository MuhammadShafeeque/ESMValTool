...
def _add_masks_albedolandcover(model_data, this_models_xxfracs, cfg):
    ...
    month_string = template_time.units.num2date(template_time.points)[
        0
    ].strftime("%b")
    ...
    masksavename = f"{month_string}-{model_attr_name}"
...
def _reconstruct_albedo_pixel(x_0, y_0, lc_logical):
    ...
    if (
        len(y_0) > np.sum(lc_logical.astype(int)) + 1
        and np.sum(lc_logical.astype(int)) > 0
    ):
        ...
        for i_0 in range(3):
            if lc_logical[i_0]:
                alb_lc_pixel[i_0] = intercept + coefficients[lc_reg] * 100.0
                lc_reg = lc_reg + 1
    return alb_lc_pixel
...
def _prepare_data_for_linreg(model_data, islice, jslice):
    ...
    for i_0 in range(3):
        ...
        for varkey in current_class:
            lc_flattened[varkey] = (
                model_data[varkey].data[islice, jslice].compressed()
            )
        lc_sum = sum([lc_flattened[varkey] for varkey in current_class])
        # Now check thresholds
        if np.var(lc_sum) > 0.0 and len(lc_sum) >= config["params"]["mingc"]:
...
def _get_reconstructed_albedos(model_data, cfg):
    ...
    for indices, maskbool in np.ndenumerate(model_data["alb"].data.mask):
        if not maskbool:  # Only if not masked we need to check neighbourhood
            ...
            islice = slice(
                int(i - (cfg["params"]["lonsize_BB"] - 1) / 2),
                int(i + (cfg["params"]["lonsize_BB"] - 1) / 2 + 1),
            )
            jslice = slice(
                int(j - (cfg["params"]["latsize_BB"] - 1) / 2),
    ...
    for ikey, jkey in it.product(result_dict.keys(), result_dict.keys()):
        if not ikey == jkey:
            ...
            transition_cube.attributes["plottitle"] = (
                month_string + "-" + datadict["alb"]["dataset"]
            )
            transition_cube.attributes["plotsuptitle"] = transition_cube.name()
            savename_nc = os.path.join(cfg["work_dir"], f"{basename}.nc")
...
def _plot_cube(cube, cfg):
    ...
    iris.quickplot.pcolormesh(cube, vmin=-0.24, vmax=0.24, cmap="bwr")
...
def main(cfg):
    ...
    for dataset_name in my_files_dict:
        ...
        if dataset_name == "Duveiller2018":
            ...
            cube.attributes["plottitle"] = (
                cube.coord("time")
                .units.num2date(cube.coord("time").points)[0]
                .strftime("%b")
                + "-"
                + "Duveiller2018"
            )
