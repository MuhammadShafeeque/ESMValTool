...
def write_plotdata(infos, modnam, values):
    ...
    filehead = {
        "area": "Accumulated land coverage for "
        + var
        + " in different regions [1.0e+6 km2]",
        "frac": "Average land cover fraction for "
        + var
        + " in different regions [%]",
        "bias": "Bias in average land cover fraction for "
        + var
...
def compute_landcover(var, lcdata, cubes):
    ...
    for sub_cube in cubes:
        ...
        coverarea.data *= 0.01 * cellarea.data / 1.0e6 / 1.0e6
...
def main(cfg):
    ...
    timcubes = {
        "exp": {key: [] for key in diag.Variables(cfg).short_names()},
        "ref": {key: [] for key in diag.Variables(cfg).short_names()},
    }
