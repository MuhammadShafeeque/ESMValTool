...
def main(cfg):
    ...
    for filename, attributes in cfg["input_data"].items():
        logger.info(
            "Processing variable %s from dataset %s",
...
def writexls(cfg, filename, ncts, nclon1, nclat1):
    ...
    for row in range(ncts.shape[1]):
        worksheet.write(
            1,
            row + 1,
            f"{round(float(nclon1[row]), 3):.3f}_\
...
def shapeselect(cfg, cube):
    ...
    if cube.coord("latitude").ndim == 1 and cube.coord("longitude").ndim == 1:
        coordpoints = [
            (x, y)
            for x in cube.coord("longitude").points
            for y in cube.coord("latitude").points
        ]
        for i, crd in enumerate(coordpoints):
            if crd[0] > 180:
                coordpoints[i] = (coordpoints[i][0] - 360.0, coordpoints[i][1])
    else:
