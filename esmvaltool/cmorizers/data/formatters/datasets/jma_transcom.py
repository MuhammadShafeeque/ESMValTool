...
def _get_coords(ctl):
    ...
    # Latitude
    lats = float(ctl["y_start"]) + (
        float(ctl["y_delta"]) * np.arange(ctl["y_size"])
    )
    ...
    # Longitude
    lons = float(ctl["x_start"]) + (
        float(ctl["x_delta"]) * np.arange(ctl["x_size"])
    )
...
def _read_control_file(file_dir, cfg):
    ...
    with open(ctl_path) as ctl_file:
