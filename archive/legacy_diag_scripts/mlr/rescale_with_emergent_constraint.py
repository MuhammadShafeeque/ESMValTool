...
def get_constraint(x_data, y_data, x_ref, x_ref_err):
    ...
    logger.info(
        "Observational constraint on '%s': (%.3f ± %.3f) %s",
        feature,
        x_ref,
        x_ref_err,
        feature_units,
    ...
    logger.info(
        "Constraint on target variable '%s': (%.3f ± %.3f) %s",
        label,
        y_mean,
        y_std,
        label_units,
    ...
    logger.info(
        "R2 of emergent relationship: %.3f (p = %.4f)",
        lines["rvalue"] ** 2,
        lines["pvalue"],
    )
    return (y_mean, y_std)
...
def rescale_labels(cfg, y_data, y_mean, y_std):
    ...
    for dataset in labels_to_rescale:
        ...
        logger.info(
            "Rescaling '%s' with factor %.2f",
            dataset["group__for__rescaling"],
            rescaling_factor,
        )
