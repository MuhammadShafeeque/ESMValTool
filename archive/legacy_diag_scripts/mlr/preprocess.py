...
def _apply_trend_aggregator(cfg, cube, data, coord_name):
    ...
    cube_slices = [
        cube.extract(iris.Constraint(**{coord_name: val}))
...
def _get_ref_calc(cfg, dataset, ref_datasets, ref_option):
    ...
    if suffix is not None:
        ...
        exp = (
            ""
            if ref_dataset.get("exp") is None
            else f", experiment {ref_dataset['exp']}"
        )
...
def _return_stderr(cfg, data):
    ...
    return (
        data.get("var_type") == "prediction_input"
...
def aggregate_by(cfg, cube, data):
    ...
    for aggregator, coords in cfg.get("aggregate_by", {}).items():
        ...
        iris_op = AGGREGATORS[aggregator]
        logger.debug(
            "Aggregating over coordinate(s) %s by calculating %s",
            coords,
            aggregator,
        )
...
def get_ref_cube(input_data, **kwargs):
    ...
    logger.info(
        "Using keyword arguments %s to extract reference datasets for "
...
def mask(cfg, cube):
    ...
    if diff:
        logger.info(
            ...
            "non-masked values, after: %i non-masked values",
            diff,
            cfg["mask"],
            n_total - n_masked_values_old,
...
def write_cube(cfg, cube, data):
    ...
    caption = (
        f"{cube.long_name} for {mlr.get_alias(data)} preprocessed with "
        f"operations {opts}."
    )
    record = {
        "ancestors": ancestors,
