...
class MLRModel:
    ...
    def __init__(self, input_datasets, **kwargs):
        ...
        **kwargs
        ...
        logger.info(
            "Initialized MLR model (using at most %i processes)",
            self._cfg["n_jobs"],
        )
    ...
    def efecv(self, **kwargs):
        ...
        **kwargs : keyword arguments, optional
        ...
        logger.info(
            "Successfully fitted MLR model on %i training point(s)",
            len(self.data["train"].index),
        )
    ...
    def plot_1d_model(self, filename=None, n_points=1000):
        ...
        logger.info(
            "Plotting 1D MLR model (sampling %i points for single "
            "feature '%s')",
            n_points,
            feature,
        )
        ...
        # Plot appearance
        title = (
            f"Predicted {self.label} by MLR model "
    ...
    def plot_partial_dependences(self, filename=None):
        ...
        for feature_name in self.features:
            ...
            title = (
                f"Partial dependence of {self.label} on {feature_name} "
            ...
            # Save plot
            new_filename = (
                filename.format(feature=feature_name)
                + "."
    ...
    def plot_prediction_errors(self, filename=None):
        ...
        axes.set_ylim(lims)
        title = (
            f"Prediction errors of {self.label} "
            f"({self._cfg['mlr_model_name']})"
        )
        axes.set_title(title)
    ...
    def plot_residuals_histogram(self, filename=None):
        ...
        title = (
            f"Histogram for residuals of {self.label} "
            f"({self._cfg['mlr_model_name']})"
        )
        axes.set_title(title)
    ...
    def plot_residuals_distribution(self, filename=None):
        ...
        axes.axvline(0.0, linestyle="--", color="k", alpha=0.75)
        title = (
            f"Probability distribution of residuals of {self.label} "
            f"({self._cfg['mlr_model_name']})"
        )
        axes.set_title(title)
        axes.set_xlabel(f"Residuals of {self._get_plot_label()}")
    ...
    def plot_scatterplots(self, filename=None):
        ...
        for feature in self.features:
            ...
            for group_attr in self.group_attributes:
                ...
                axes.plot(
                    self.data["all"].x.loc[group_attr, feature],
        ...
        **kwargs : keyword arguments, optional
        ...
        for pred_name in self._datasets["prediction_input"]:
            ...
            pred_dict = self._get_prediction_dict(
                ...
                get_propagated_errors=save_propagated_errors,
                **kwargs,
            )
    ...
    def rfecv(self, **kwargs):
        ...
        **kwargs : keyword arguments, optional
        ...
        logger.info(
            ...
            "with final regressor %s on %i training points",
            self._CLF_TYPE,
            len(self.data["train"].index),
        )
    ...
    def _extract_x_data(self, datasets, var_type):
        ...
        if sample_weights_for_groups is not None:
            ...
            if (
                sample_weights.max().values[0] / sample_weights.min().values[0]
            ) > 150.0:
                logger.warning(
    ...
    def _get_broadcasted_cube(self, dataset, ref_cube, text=None):
        ...
        logger.info(
            "Broadcasting %s from %s to %s",
            msg,
            data_to_broadcast.shape,
            target_shape,
        )
    ...
    def _get_cv_estimator_kwargs(self, cv_estimator, **kwargs):
        ...
        logger.info(
            "Using keyword argument(s) %s for class %s",
            cv_kwargs,
            cv_estimator,
        )
    ...
    def _get_label(self):
        ...
        logger.info(
            "Found label '%s' with units '%s' (defined in 'label' data)",
            labels[0],
            units,
        )
    ...
    def _get_mask(self, x_data, data_type):
        ...
        if self._cfg["imputation_strategy"] == "remove":
            ...
            logger.debug(
                ...
                "missing (because imputation_strategy = 'remove')",
                mask.sum(),
                data_type,
            )
        ...
        for attr in ("dataset", "exp", "project", "start_year", "end_year"):
            ...
            if len(attrs) > 1:
                ...
                logger.debug(
                    ...
                    "%s, using '%s' for prediction cubes",
                    attr,
                    attrs,
                    properties[attr],
                )
    ...
    def _get_verbosity_parameters(self, function, boolean=False):
        ...
        for param, log_levels in verbosity_params.items():
            ...
            if param in all_params:
                ...
                logger.debug(
                    "Set verbosity parameter '%s' of %s to '%s'",
                    param,
                    str(function),
                    parameters[param],
                )
    ...
    def _get_x_data_for_group(self, datasets, var_type, group_attr=None):
        ...
        logger.debug(
            "Found %i raw '%s' input data points%s",
            len(group_data.index),
            var_type,
            msg,
        )
    ...
    def _load_data(self):
        ...
        if test_size:
            ...
            logger.info(
                ...
                int(test_size * 100),
                len(self.data["test"].index),
            )
            logger.info(
                "%i point(s) remain(s) for training",
                len(self.data["train"].index),
            )
        else:
            self._data["train"] = self.data["all"].copy()
            logger.info(
                "Using all %i input data point(s) for training",
                len(y_all.index),
            )
    ...
    def _load_input_datasets(self, input_datasets):
        ...
        # Check datasets
        msg = (
            "At least one '{}' dataset does not have necessary MLR attributes"
        )
        datasets_to_check = {
        ...
        logger.info(
            ...
            len(label_datasets),
            len(pred_in_datasets),
            len(pred_in_err_datasets),
            len(pred_ref_datasets),
        )
        labeled_datasets = {
            "Feature": feature_datasets,
            "Label": label_datasets,
    ...
    def _pred_type_to_metadata(self, pred_type, cube):
        ...
        error_types = {
            "var": " (variance)",
            "cov": " (covariance)",
            "squared_mlr_model_error_estim": (
                " (squared MLR model error "
                "estimation using hold-out test "
                "data set)"
            ),
            "squared_propagated_input_error": (
                " (squared propagated error of "
        ...
        if pred_type is None:
            ...
            long_name = (
                f"Local importance of feature {feature} for "
    ...
    def _remove_missing_features(self, x_data, y_data, sample_weights):
        ...
        diff = mask.sum()
        if diff:
            msg = "Removed %i training point(s) where features were missing"
            if self._cfg.get("accept_only_scalar_data"):
                removed_groups = self.group_attributes[mask]
                msg += f" ({removed_groups})"
                self._classes["group_attributes"] = self.group_attributes[
                    ~mask
                ]
    ...
    def _save_prediction_cubes(self, pred_dict, pred_name, x_cube):
        ...
        for pred_type, y_pred in pred_dict.items():
            ...
            record = {
                ...
                "caption": (
                    f"{pred_cube.long_name} of MLR model "
    ...
    def _set_prediction_cube_attributes(self, cube, pred_type, pred_name=None):
        ...
        sub_str = (
            ""
            if self._cfg["sub_dir"] == ""
            else f"_of_group_{self._cfg['sub_dir']}"
        )
        filename = (
            f"{self.mlr_model_type}_{self.label}_prediction{suffix}"
    ...
    def _update_random_state_parameter(self, function, parameters):
        ...
        all_params = (
            getfullargspec(function).args + getfullargspec(function).kwonlyargs
        )
    ...
    def _convert_units_in_cube(cube, new_units, power=None, text=None):
        ...
        if power:
            logger.debug(
                "Raising target units of cube '%s' by power of %i",
    ...
    def _get_coordinate_data(ref_cube, var_type, tag, text=None):
        ...
        if coord_dims == ():
            logger.warning(
                ...
                "add any information to the model (array is constant)",
                tag,
            )
    ...
    def _set_axis_lim_symmetric(axes, axis):
        ...
        if axis == "x":
            getter = axes.get_xlim
            setter = axes.set_xlim
        elif axis == "y":
            getter = axes.get_ylim
            setter = axes.set_ylim
        else:
