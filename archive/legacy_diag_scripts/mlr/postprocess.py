...
def _calculate_real_error(cfg, ref_cube, cov_cube, basepath):
    ...
    logger.info(
        "Real error (using covariance): %s %s",
        real_error.data,
        real_error.units,
    )
...
def _estim_cov_differing_shape(cfg, squared_error_cube, cov_est_cube, weights):
    ...
    logger.info(
        ...
        "('prediction_output_error') with shape %s",
        cov_est_cube.shape,
...
def _estim_cov_identical_shape(squared_error_cube, cov_est_cube, weights):
    ...
    # Covariances
    cov_dim0 = (
        np.einsum("...i,...j->...ij", error, error)
        * np.einsum("...i,...j->...ij", weights, weights)
        * pearson_dim1
    )
    cov_dim1 = (
        np.einsum("i...,j...->...ij", error, error)
...
def _estimate_real_error(cfg, squared_error_cube, cov_est_dataset, basepath):
    ...
    io.iris_save(real_error, new_path)
    logger.info(
        "Estimated real error (using estimated covariance): %s %s",
        real_error.data,
        real_error.units,
    )
...
def _get_all_weights(cfg, cube, power=1):
    ...
    for operation in ("sum", "mean"):
        ...
        weights /= (
            _get_normalization_factor(None, coords, cube, normalize=normalize)
            ** power
        )
...
def _get_covariance_dataset(error_datasets, ref_cube):
    ...
    explanation = (
        "i.e. dataset with short_name == '*_cov' among "
...
def split_datasets(datasets, tag, pred_name):
    ...
    logger.info(
        ...
        "prediction '%s': %s (used as reference)",
        tag,
        pred_name,
        mean[0]["filename"],
    )
    ...
    if not error:
        ...
        if not cov_estimation:
            ...
            logger.info(
                ...
                "estimation for tag '%s' of prediction '%s': %s",
                tag,
                pred_name,
